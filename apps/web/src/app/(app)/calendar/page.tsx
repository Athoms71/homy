...
const DOT_CLASSES: Record<EventColor, string> = {
  ...
  yellow: 'bg-yellow-500', gray: 'bg-gray-400',
}

const RECURRENCE_LABELS: Record<RecurrenceFreq, string> = {
  none: 'Pas de répétition', daily: 'Chaque jour',
  weekly: 'Chaque semaine', monthly: 'Chaque mois', yearly: 'Chaque année',
}
...
function EventBar({ event, onClick, isStart, isEnd, isMultiDay }: {
  event: any; onClick: (e: any) => void
  isStart: boolean; isEnd: boolean; isMultiDay: boolean
}) {
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isMultiDay
          ? cn(c.bar, 'text-white px-1', isStart ? 'rounded-l-full pl-2' : '', isEnd ? 'rounded-r-full pr-2' : '')
          : cn(c.bg, c.text, 'rounded-full px-1.5'),
      )}
    >
      {isStart && !event.all_day && (
        <span className={cn('opacity-80 mr-0.5 shrink-0', isMultiDay ? 'text-white' : '')}>
...
function EventModal({ householdId, currentUserId, members, event, defaultDate, onClose }: {
  householdId: string; currentUserId: string; members: any[]
  event?: any | null; defaultDate?: Date; onClose: () => void
}) {
    ...
    <>
      <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 px-0 sm:px-4">
        <div className="bg-white rounded-t-3xl sm:rounded-2xl w-full sm:max-w-lg shadow-xl max-h-[88vh] overflow-y-auto pb-safe">
          ...
          <div className="px-5 py-4 space-y-4">
            <input
              type="text" placeholder="Titre de l'événement" value={title}
            ...
            <div>
              ...
              <input type="text" placeholder="Adresse ou lieu" value={location}
                onChange={e => setLocation(e.target.value)} disabled={!canEdit}
                className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2 focus:border-primary-500 outline-none"
              />
            </div>
            ...
            <textarea placeholder="Description (optionnel)" value={description}
              onChange={e => setDescription(e.target.value)} disabled={!canEdit} rows={2}
            ...
            {canEdit && (
              <div>
                ...
                <select value={recurrence} onChange={e => setRecurrence(e.target.value as RecurrenceFreq)}
      ...
      {confirmDelete && (
        <ConfirmDialog
          ...
          onConfirm={async () => { await remove.mutateAsync(event!.id); onClose() }}
...
function MonthView({ currentDate, events, today, onDayClick, onEventClick }: {
  currentDate: Date; events: any[]; today: Date
      ...
      <div className="grid grid-cols-7 border border-gray-100 rounded-2xl overflow-hidden">
        {days.map((day, i) => {
          ...
          return (
            ...
            >
              <span className={cn(
                'text-xs font-semibold w-6 h-6 flex items-center justify-center rounded-full mx-auto mb-0.5',
              ...
              <div className="space-y-0.5">
                {visible.map(ev => {
                  ...
                  return (
                    <EventBar key={ev.id} event={ev} onClick={onEventClick}
                      isStart={isStart} isEnd={isEnd} isMultiDay={isMultiDay}
                    />
                  )
                })}
                {hidden > 0 && (
                  <div className="text-xs text-gray-400 pl-1">+{hidden} autre{hidden > 1 ? 's' : ''}</div>
                )}
              </div>
            </div>
          )
...
function DayView({ day, events, onEventClick, onSlotClick }: {
  day: Date; events: any[]; onEventClick: (e: any) => void; onSlotClick: (h: number) => void
}) {
        ...
        {hours.map(h => {
          ...
          return (
            <div key={h} onClick={() => onSlotClick(h)}
              className="flex gap-2 border-b border-gray-50 last:border-0 min-h-[52px] cursor-pointer hover:bg-gray-50 transition px-3 py-1"
            >
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">{formatTime(ev.starts_at)} – {formatTime(ev.ends_at)}</p>
                      </div>
                    </div>
                  )
                })}
  ...
  return (
    <div>
      ...
      {isLoading ? (
        ...
        <MonthView
          currentDate={currentDate} events={events} today={today}
          onDayClick={handleDayClick} onEventClick={handleEventClick}
        />
      ) : (
        <DayView
          day={currentDate} events={events}
      ...
      <button
        ...
        className="sm:hidden fixed bottom-20 right-4 w-14 h-14 bg-primary-600 text-white rounded-full shadow-lg flex items-center justify-center z-40"
      >
        <Plus size={24} />
      </button>
      ...
      {showModal && householdId && currentUserId && (
        <EventModal
          householdId={householdId} currentUserId={currentUserId} members={membersData}
          event={selectedEvent} defaultDate={modalDefaultDate}
