  ...
  return (
    <button onClick={() => !disabled && onChange(!checked)} disabled={disabled}
      className={cn("relative inline-flex h-6 w-11 items-center rounded-full transition-colors",
        checked ? "bg-primary-600" : "bg-gray-200", disabled && "opacity-50 cursor-not-allowed"
      )}
    >
...
function HouseholdSection() {
  ...
  return (
    <>
            ...
            <div key={household.id} className="border border-gray-100 rounded-2xl overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 bg-gray-50">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">{isAdmin ? "👑 Admin" : "👤 Membre"} · Code : {household.invite_code}</p>
                </div>
              </div>
              <div className="divide-y divide-gray-50">
                <button onClick={() => setConfirmLeaveId(household.id)} disabled={leaving}
                ...
                {(isAdmin || isCreator) && (
                  <button onClick={() => setConfirmDeleteId(household.id)} disabled={deleting}
      ...
      {confirmLeaveId && (
        <ConfirmDialog
          message="Quitter ce foyer ? Tu perdras accès à toutes ses données."
          confirmLabel="Quitter"
          onConfirm={async () => {
            setLeaving(true);
            await leaveHousehold(confirmLeaveId);
      ...
      {confirmDeleteId && (
        <ConfirmDialog
          message="Supprimer définitivement ce foyer ? Cette action est irréversible."
          confirmLabel="Supprimer"
          onConfirm={async () => {
            setDeleting(true);
            await supabase.from("households").delete().eq("id", confirmDeleteId);
