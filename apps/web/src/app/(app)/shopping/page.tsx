...
export default function ShoppingPage() {
  ...
  function handleCreateList(e: React.FormEvent) {
    ...
    createList.mutate(newListName.trim(), {
      onSuccess: (list) => { setNewListName(''); setShowCreate(false); setActiveList(list) }
    })
  }
  ...
  if (!householdsData || householdsData.length === 0) return (
  ...
  if (activeList && householdId) return (
    <ShoppingListView list={activeList} householdId={householdId} onBack={() => setActiveList(null)} />
  )
    ...
    <div>
      ...
      {householdsData.length > 1 && (
        <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
          {householdsData.map((item: any) => (
            <button key={item.households.id} onClick={() => setHouseholdId(item.households.id)}
              className={cn('flex-shrink-0 px-3 py-1.5 rounded-full text-sm font-medium transition',
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50 px-4">
          <div className="bg-white rounded-t-3xl sm:rounded-2xl w-full max-w-md p-6 shadow-xl pb-8 sm:pb-6">
            <h2 className="text-lg font-bold text-gray-900 mb-4">🛒 Nouvelle liste</h2>
            <form onSubmit={handleCreateList} className="space-y-4">
              <input type="text" value={newListName} onChange={e => setNewListName(e.target.value)}
                placeholder="Courses de la semaine, Fête..." autoFocus required
      ...
      {confirmDeleteId && (
        <ConfirmDialog
          ...
          onConfirm={() => { deleteList.mutate(confirmDeleteId); setConfirmDeleteId(null) }}
