...
import { CalendarWidget } from "@/components/modules/dashboard/CalendarWidget";
...
function KpiCard({ emoji, label, value, sub, color, href }: { emoji: string; label: string; value: string; sub?: string; color: "blue" | "green" | "orange" | "red" | "purple"; href: string }) {
	const colors = {
...
function KpiRow({ householdId, currentUserId, memberIds }: { householdId: string; currentUserId: string; memberIds: string[] }) {
	...
	const iOweTotal = balances.filter((b) => b.fromUserId === currentUserId).reduce((sum, b) => sum + b.amount, 0);
	...
	return (
		<div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
			...
			<KpiCard
				...
				href="/bills"
			/>
			<KpiCard emoji="📊" label="Budget mensuel" value={`${monthly.toFixed(0)} €`} sub="Factures récurrentes" color="purple" href="/bills" />
		</div>
	);
}
		...
		<div className="bg-orange-50 border border-orange-200 rounded-2xl px-4 py-3 flex items-center gap-3 mb-5">
			...
			<p className="text-orange-700 text-sm">
				<span className="font-semibold">
					{critical.length} facture{critical.length > 1 ? "s" : ""}
				</span>{" "}
				nécessite{critical.length === 1 ? "" : "nt"} ton attention —{" "}
				<Link href="/bills" className="underline font-medium">
					voir les factures
				</Link>
			</p>
		</div>
...
export default function DashboardPage() {
	...
	useEffect(() => {
		...
		if (hour < 18) setGreeting("Bonjour");
	...
	return (
		<div>
			...
			<div className="mb-6">
				...
				<p className="text-gray-400 text-sm mt-0.5">{new Date().toLocaleDateString("fr-FR", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}</p>
			</div>
			...
			{householdId && currentUserId ? (
				<>
					...
					{/* KPIs */}
					<KpiRow householdId={householdId} currentUserId={currentUserId} memberIds={memberIds} />

					{/* Desktop : Courses | Calendrier | Soldes + Factures */}
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						...
						{/* Col 3 — Soldes + Factures */}
						<div className="md:col-span-1 flex flex-col gap-4">
							<ExpensesWidget householdId={householdId} currentUserId={currentUserId} memberIds={memberIds} />
