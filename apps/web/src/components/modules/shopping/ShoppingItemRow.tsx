...
export function ShoppingItemRow({ item, listId }: Props) {
	...
	return (
		<div className={cn("flex items-center gap-3 px-4 py-3 rounded-xl transition", isChecked ? "bg-gray-50 opacity-60" : "bg-white hover:bg-gray-50")}>
			{/* Checkbox */}
			<button
			...
			<div className="flex-1 min-w-0">
				<span className={cn("text-sm font-medium", isChecked ? "line-through text-gray-400" : "text-gray-800")}>{item.name}</span>
			...
			<button onClick={() => remove.mutate(item.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-300 hover:text-red-400 transition flex-shrink-0">
				<Trash2 size={15} />
			</button>
		</div>
	);
