...
import { EVENT_COLOR_CLASSES } from "@homy/shared-types";
import type { EventColor } from "@homy/shared-types";
			...
			{isLoading ? (
				...
				<div className="space-y-2">
					{events.map((event: any) => {
						...
						const colorClasses = EVENT_COLOR_CLASSES[color];
						...
						return (
							<Link key={event.id} href="/calendar" className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 transition hover:opacity-80", colorClasses.bg)}>
								{/* Barre couleur */}
								<div className={cn("w-1 h-8 rounded-full flex-shrink-0", colorClasses.bg.replace("-100", "-400"))} />
