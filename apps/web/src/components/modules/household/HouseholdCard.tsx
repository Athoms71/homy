...
interface Props {
	household: { id: string; name: string; invite_code: string; created_by: string };
	role: string;
...
export function HouseholdCard({ household, role, currentUserId }: Props) {
	...
	return (
		<>
			<div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
				...
				<div className="bg-primary-600 px-5 py-4 flex items-center justify-between">
					<div>
						...
						<span className="text-primary-200 text-xs capitalize">{isAdmin ? "👑 Admin" : "👤 Membre"}</span>
					</div>
					<button onClick={() => setConfirmLeave(true)} disabled={leave.isPending} className="text-primary-200 hover:text-white transition p-1" title="Quitter le foyer">
						<LogOut size={18} />
					</button>
				</div>
				...
				<div className="px-5 py-4 border-b border-gray-100">
					...
					<div className="flex items-center gap-2">
						<span className="font-mono text-xl font-bold tracking-widest text-primary-700 bg-primary-50 px-4 py-2 rounded-lg flex-1 text-center">{household.invite_code}</span>
						<button onClick={copyCode} className="p-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition" title="Copier">
							{copied ? <Check size={18} className="text-green-600" /> : <Copy size={18} className="text-gray-600" />}
						</button>
						{isAdmin && (
							<button onClick={() => setConfirmRegen(true)} disabled={regenerate.isPending} className="p-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition" title="Nouveau code">
			...
			{confirmLeave && (
				<ConfirmDialog
					message={`Quitter le foyer "${household.name}" ?`}
			...
			{confirmRegen && (
				<ConfirmDialog
					message="Générer un nouveau code ? L'ancien ne fonctionnera plus."
