...
export function CreateHouseholdForm({ onSuccess }: Props) {
	...
	return (
		<form onSubmit={handleSubmit} className="space-y-4">
			...
			{error && <p className="text-red-600 text-sm bg-red-50 rounded-lg px-3 py-2">{(error as Error).message}</p>}
			...
			>
				{isPending ? <Loader2 size={18} className="animate-spin" /> : <Home size={18} />}
