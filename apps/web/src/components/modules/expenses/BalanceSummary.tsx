...
export function BalanceSummary({ balances, members, currentUserId, groupId, expenses }: Props) {
	...
	function findSplitId(fromUserId: string, toUserId: string) {
		for (const expense of expenses) {
			...
			const split = expense.expense_splits?.find((s: any) => s.user_id === fromUserId && !s.is_paid);
	...
	return (
		<div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
			<div className="px-5 py-3 bg-gray-50 border-b border-gray-100">
				<h3 className="font-semibold text-gray-700 text-sm">Bilan</h3>
			</div>
					...
					return (
						<div key={i} className={cn("flex items-center gap-3 px-5 py-3.5", isMe && "bg-orange-50")}>
							<div className="flex-1 flex items-center gap-2 min-w-0">
								<span className={cn("text-sm font-semibold", isMe ? "text-orange-700" : "text-gray-700")}>{getName(b.fromUserId)}</span>
								<ArrowRight size={14} className="text-gray-400 flex-shrink-0" />
								<span className="text-sm text-gray-600">{getName(b.toUserId)}</span>
							</div>
							<span className={cn("font-bold text-sm flex-shrink-0", isMe ? "text-orange-600" : "text-gray-700")}>{b.amount.toFixed(2)} €</span>
							{isMe && splitId && (
								<button
